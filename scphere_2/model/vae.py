...
class SCPHERE(object):
    def __init__(self, x, batch, n_gene, n_batch=None, z_dim=2,
        ...
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, n_gene], name='x')
        ...
        with tf.compat.v1.name_scope('ELBO'):
            ...
            tf.compat.v1.add_to_collection('depth_loss', self.depth_loss)
            tf.compat.v1.add_to_collection('log_likelihood', self.log_likelihood)
            tf.compat.v1.add_to_collection('kl', self.kl)
            tf.compat.v1.add_to_collection('elbo', self.ELBO)
    ...
    def _depth_regularizer(self, batch):
        with tf.compat.v1.name_scope('depth-regularizer'):
            samples = tf.random.poisson(lam=self.x * 0.2, shape=[1], seed=0)
            ...
            z_mu1, z_sigma_square1 = self._encoder(tf.nn.relu(self.x - samples), batch)
...
class OptimizerVAE(object):
    ...
    def __init__(self, model, learning_rate=0.001, depth_loss=True):
        ...
        with tf.compat.v1.name_scope('gradient-clip'):
            ...
            grad = tf.compat.v1.gradients(self.loss, trainable_variable)
        ...
        tf.compat.v1.add_to_collection(tf.compat.v1.GraphKeys.LOSSES, 1)
